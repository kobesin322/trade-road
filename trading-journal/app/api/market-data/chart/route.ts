...
import { fetchYahooMarketChart } from "@/lib/market-data/yahoo-chart";
...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json(await fetchYahooMarketChart(item, { range, interval }));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Chart data unavailable.";
